...
public class CircleView extends View {
    ...
    public int mCircleMode = CIRCLE_MODE_NORMAL;//圆环模式 默认为普通模式

    private static final int EXTRA_CIRCLE_WIDTH = 1;
    public boolean mOuterCircleIsShow = false;//外部圆环是否显示  默认不显示
    public int mOuterCirclePad = 20;//外部圆环距离主圆环边距
    private Paint extraCirclePaint = new Paint();

    public boolean mInnerCircleIsShow = false;//内装饰圆是否显示
    public int mInnerCirclePad = 20;

    public int stokenWidth = 20;//圆环宽度
    private int angle = 0;
    private Paint paint = new Paint();//绘制主圆环画笔
    private RectF ovalRect = new RectF();
    public int mRadiusColor =0xfff5f5f5;//圆环颜色

    public boolean isBottomCircleShow = false;//底部圆是否显示  默认不显示
    ...
    public int startRotateAngle = MIN_VALUE;//圆盘开始角度 默认为最底部0度
    public int angleRotateSpan = MAX_VALUE;//圆盘可旋转角度范围

    private RectF tempRect = new RectF();
    LinearGradient lg = new LinearGradient(0, 0, 360, 360, 0xff3892d9, 0xfff0d93c,Shader.TileMode.CLAMP);
...
//        for (int i = 0, end = 360; i < end; i++) {
//            paint.setColor(getGradient(i / (float) end, R.color.spark_btn_enter_bg, R.color.colorAccent));
//        }
    ...
    public void setIndicatorBit(Bitmap bit) {
        indicatorBitmap = bit;
    ...
    //内圆设置
    public void setInnerCircle(boolean isShow, int pad) {
        this.mInnerCircleIsShow = isShow;
        mInnerCirclePad = mInnerCircleIsShow ? pad : 0;
    }

    //外圆设置
    public void setOuterCircle(boolean isShow, int pad) {
...
//        for (int i = 0; i < 360; i++) {
//            paint.setColor(getGradient(i,R.color.spark_btn_enter_bg, R.color.colorAccent));
//        }
        paint.setShader(lg);
//        paint.setColor(mRadiusColor);
    ...
    //设置圆环模式
    public void setCircleMode(int mode) {
        mCircleMode = mode;
        switch (mCircleMode) {
            ...
            case CIRCLE_MODE_DOT:
                PathEffect effects = new DashPathEffect(new float[]{5, 10, 5, 10}, 5);
    ...
    //设置底部圆是否显示
    public void setBottomCircleShow(boolean isShow) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.save();
        canvas.rotate(270 + startRotateAngle, centerX, centerY);//坐标系旋转
...
//        //draw Main
        ...
        if (mInnerCircleIsShow) {//内圆显示
            ...
            canvas.drawArc(tempRect, 0, angleRotateSpan, false, paint);
        }
...
//            //圆的参数方程确定坐标点 极坐标
//            int x = (int) (radius * Math.cos(radAngle)) + centerX - (int) (bitDistRect.width());
//            int y = (int) (radius * Math.sin(radAngle)) + centerY - (int) (bitDistRect.height());
